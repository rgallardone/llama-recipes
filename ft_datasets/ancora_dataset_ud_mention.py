...
from .coref_instructions import INSTRUCTIONS_MENTION

RANDOM_SEED = 42
POSTPROCESSED_DATASET_DIR = "postprocessed/mention_detection"


class AncoraDatasetUDMention(torch.utils.data.Dataset):
    train_dataset = None
    dev_dataset = None
            ...
            for sent_str in sentence_strings[1:]:
                ...
                while line_index < len(lines[1:]) + 1:
                    ...
                    if "Entity" in misc_str_start:
                        ...
                        entity_starts = re.findall(r"\(e\d+-.*?-\d+", entity_string)

                        for e_str in entity_starts:
                            head_word_index = re.search(r"\d+$", e_str).group(0)
                            sentence_gold_text += f"[head{head_word_index} "
    ...
    def __init__(
        ...
        if AncoraDatasetUDMention.train_dataset is None:
            datasets_dir = f"{dataset_config.data_dir}/{POSTPROCESSED_DATASET_DIR}"
            ...
            AncoraDatasetUDMention.train_dataset = Dataset.from_pandas(
                train_dataset_df, split="train"
            )
            AncoraDatasetUDMention.dev_dataset = Dataset.from_pandas(
                dev_dataset_df, split="dev"
            )

        if partition == "train":
            self.dataset_split = AncoraDatasetUDMention.train_dataset
        else:
            self.dataset_split = AncoraDatasetUDMention.dev_dataset
        self.tokenizer = tokenizer
        self.inst = INSTRUCTIONS_MENTION["inst"]
        self.inst_first = INSTRUCTIONS_MENTION["inst_first"]
